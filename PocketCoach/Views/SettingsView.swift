    ...
    private var mainSettingsView: some View {
            VStack(spacing: 32) {
                VStack(spacing: 24) {
                    ...
                    Button(action: {
                        let newMode = nextAppearanceMode(after: appearanceMode)
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
                            appearanceMode = newMode
                        }
                        Analytics.appearanceChanged(to: newMode)
                    }) {
                        SettingsNoteRow(
                            title: "Appearance",
                    ...
                    Button(action: {
                        let newMode = conversationMode == "solo" ? "couple" : "solo"
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.8)) {
                            conversationMode = newMode
                        }
                        Analytics.conversationModeChanged(to: newMode)
                    }) {
                        SettingsNoteRow(
                            title: "Mode",
                ...
                // Footer
                VStack(spacing: 8) {
                    Link(destination: URL(string: "https://getpocketcoach.app/support")!) {
                        ...
                        Button(action: toggleRecording) {
                            HStack(spacing: 8) {
                                Image(systemName: isRecording ? "stop.circle.fill" : "mic.circle.fill")
                                    .font(.system(size: 28))
                                    .foregroundColor(isRecording ? .red : Constants.coachingPrimaryColor)
