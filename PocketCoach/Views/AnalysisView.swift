    ...
    private func coachSection(coaching: CoachingAnalysis) -> some View {
        ...
        cardSection {
            VStack(alignment: .leading, spacing: 12) {
                sectionLabel("POST-GAME DRILL", icon: "figure.2.arms.open", color: Constants.coachingPrimaryColor)
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    Text(analyst.topicAnalysis.primaryCategory)
                        ...
                        .padding(.vertical, 5)
                        .background(Constants.coachingPrimaryColor.opacity(0.12))
                        .foregroundColor(Constants.coachingPrimaryColor)
                        .clipShape(Capsule())
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Button(action: {
                ...
                if isExpanded {
                    Analytics.coachingOpportunityExpanded(foul: opportunity.callMade)
                }
            }) {
                HStack(spacing: 10) {
                    Circle()
...
struct DrillCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if isExpanded {
                ...
                Text("INSTRUCTIONS")
                    .font(.system(size: 10, weight: .black))
                    .kerning(0.8)
                    .foregroundColor(Constants.coachingPrimaryColor.opacity(0.7))
                    .padding(.bottom, 4)
        ...
        .padding(16)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Constants.coachingPrimaryColor.opacity(colorScheme == .dark ? 0.06 : 0.03))
        .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .stroke(Constants.coachingPrimaryColor.opacity(0.1), lineWidth: 1)
        )
    }
}
