...
enum PocketCoachError: LocalizedError, Equatable {
    ...
    var failureReason: String? {
        switch self {
        case .microphonePermissionDenied:
            return "PocketCoach needs microphone access to record sessions."
    ...
    var recoverySuggestion: String? {
        switch self {
        case .microphonePermissionDenied:
            return "Go to Settings > PocketCoach > Microphone to enable access."
        case .modelDownloadFailed:
...
class ErrorHandler: ObservableObject {
    ...
    @Published var currentError: PocketCoachError?
    @Published var showingError = false
    @Published var isRecovering = false
    
    private let logger = Logger(subsystem: "com.reactnativenerd.PocketCoach", category: "ErrorHandler")
    private var errorQueue: [PocketCoachError] = []
    
    func handle(_ error: Error) {
        logger.error("Error occurred: \(error.localizedDescription)")
        
        if let coachingError = error as? PocketCoachError {
            handlePocketCoachError(coachingError)
        } else {
            // Convert to PocketCoachError
            let coachingError = mapToPocketCoachError(error)
            handlePocketCoachError(coachingError)
        }
    }
    
    private func handlePocketCoachError(_ error: PocketCoachError) {
        // Add to queue
    ...
    private func mapToPocketCoachError(_ error: Error) -> PocketCoachError {
        switch error {
    ...
    private func attemptAutomaticRecovery(for error: PocketCoachError) async {
        isRecovering = true
...
extension Notification.Name {
    static let retryFailedOperation = Notification.Name("com.reactnativenerd.pocketcoach.retryFailedOperation")
}