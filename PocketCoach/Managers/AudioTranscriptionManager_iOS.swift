...
class AudioTranscriptionManager_iOS: ObservableObject {
    ...
    @Published var isModelReady = ScreenshotMockData.isScreenshotMode
    ...
    private let bufferQueue = DispatchQueue(label: "com.reactnativenerd.pocketcoach.bufferQueue", attributes: .concurrent)
        ...
        Task {
            ...
            await MainActor.run {
                if hasPermission {
                    ...
                    currentSession = CoachingSession(startedAt: Date())
                    ...
                    #endif
                    ErrorHandler.shared.handle(PocketCoachError.microphonePermissionDenied)
                }
            }
        }
    }
