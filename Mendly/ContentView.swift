...
struct ContentView: View {
    ...
    var body: some View {
        ...
        // Model pre-loading is handled by MendlyApp.preloadModelsIfNeeded()
...
struct RecordView: View {
    ...
    private var backgroundGradient: some View {
        LinearGradient(
            gradient: Gradient(colors: [
                Constants.therapyPrimaryColor.opacity(colorScheme == .dark ? 0.08 : 0.12),
    ...
    var body: some View {
                ...
                if transcriptionManager.isDiarizing || transcriptionManager.isAnalyzing {
                    ...
                    VStack(spacing: 16) {
                        ...
                        if transcriptionManager.isTranscribing && !transcriptionManager.currentTranscription.isEmpty {
                            VStack(alignment: .leading, spacing: 16) {
                                HStack(spacing: 12) {
                                    Image(systemName: "pencil.and.outline")
                                        .foregroundColor(Constants.therapyCardForeground)
                                        .font(.system(size: 14, weight: .bold))
                                    Text("TRANSCRIPTION")
                                        .font(.system(size: 11, weight: .black))
                                        .kerning(1.2)
                                        .foregroundColor(Constants.therapyCardForeground.opacity(0.8))
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 32, style: .continuous)
                                    .stroke(Constants.therapyPrimaryColor.opacity(0.15), lineWidth: 1)
                            )
                            .padding(.horizontal, 24)
        ...
        .onChange(of: transcriptionManager.analysisPhase) { newPhase in
            if newPhase == .complete && !hasAutoShownAnalysis {
    ...
    private var invalidConversationView: some View {
        VStack(spacing: 24) {
            ...
            Button(action: {
                ...
                .shadow(color: Constants.therapyPrimaryColor.opacity(0.3), radius: 8, y: 4)
            }
            .padding(.horizontal, 24)
    ...
    private var processingProgressView: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 32) {
                ...
                ZStack {
                    Circle()
                        .stroke(Constants.therapyPrimaryColor.opacity(0.15), lineWidth: 8)
                        .frame(width: 80, height: 80)
                    Circle()
                        .trim(from: 0, to: processingProgress)
                        .stroke(
                            Constants.therapyPrimaryColor,
    ...
    private func processingStep(label: String, isActive: Bool, isComplete: Bool) -> some View {
        HStack(spacing: 12) {
            ...
            Text(label)
                ...
                .foregroundColor(isActive ? .primary : (isComplete ? Constants.therapyPrimaryColor : .secondary))
                ...
                Button(action: {
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .stroke(Constants.therapyPrimaryColor.opacity(0.2), lineWidth: 1)
                    )
                }
